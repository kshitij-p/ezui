...
const CodeCopyBlock = ({ children, ...rest }: React.ComponentPropsWithoutRef<typeof CodeBlock>) => {
  ...
  return (
    <CodeBlock {...rest}>
      <button
        className="absolute bottom-0 right-0 top-0 mr-4 mt-5 h-max w-max rounded text-light-text transition hover:text-primary focus:outline-0 focus-visible:text-primary focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:opacity-50"
        onClick={async () => {
