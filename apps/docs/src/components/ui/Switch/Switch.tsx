...
>(({ className, ...props }, ref) => (
  ...
  >
    <RadixSwitch.Thumb
      className={cn(
        "pointer-events-none block h-5 w-5 rounded-full bg-accent/50 shadow-lg ring-0 transition group-aria-[invalid=true]:bg-invalid/25 data-[state=checked]:translate-x-5 data-[state=unchecked]:translate-x-0 data-[state=checked]:bg-primary group-aria-[invalid=true]:data-[state=checked]:bg-invalid"
      )}
    />
  </RadixSwitch.Root>
));
